...
class SETTINGS:
    ...
    PROGRAM_NAME:         str  = "process_monitoring_tools.py"
    ...
    PROCESS_CREATION:     list = ( "Process Creation", "programs\\process_creation.py")
    PROCESS_DELETION:     list = ("Process Deletion", "programs\\process_deletion.py")
...
def on_clicked_Tools(icon, item):
    ...
    if str(item) == SETTINGS.PROCESS_CREATION[0]:
        program = cd + palka + SETTINGS.PROCESS_CREATION[1]
        os.startfile(program)
    elif str(item) == SETTINGS.PROCESS_DELETION[0]:
        program = cd + palka + SETTINGS.PROCESS_DELETION[1]
        os.startfile(program)
    elif str(item) == SETTINGS.PROCESS_MONITOR[0]:
        program = cd + palka + SETTINGS.PROCESS_MONITOR[1]
        os.startfile(program)
    elif str(item) == SETTINGS.PROCESS_EXPLORER[0]:
        program = cd + palka + SETTINGS.PROCESS_EXPLORER[1]
        os.startfile(program)
    elif str(item) == SETTINGS.PROCESS_RAMMAP[0]:
        program = cd + palka + SETTINGS.PROCESS_RAMMAP[1]
        os.startfile(program)
    elif str(item) == SETTINGS.PROCESS_WINOBJ[0]:
        program = cd + palka + SETTINGS.PROCESS_WINOBJ[1]
        os.startfile(program)
    elif str(item) == SETTINGS.PROCESS_AUTORUNS[0]:
        program = cd + palka + SETTINGS.PROCESS_AUTORUNS[1]
        os.startfile(program)
